...
def bill(second):
    if second < 1800:
        ...
        return 10 + math.ceil((second - 7200) / 3600) * 2
    ...
    def calculate_2(self):
        ...
        for record in copy.deepcopy(self.records):
            ...
            if record["i_o"] == "in" or not d.get(car_no):
                ...
                add_second = int(diff.total_seconds())
                stop_second += add_second
                cost_list[record["datetime"].month] += bill(add_second)
        return stop_second, {key: value for key, value in d.items() if value}, cost_list
...
class Dispatcher:
    ...
    def get_result(self):
        ...
        def cal_leave(d: dict):
            ...
            for key, value in d.items():
                ...
                for i in range(1, len(value), 2):
                    ...
                    cost_list[value[i]["datetime"].month] += bill(diff.total_seconds())
            return stop_second, cost_list
        ...
        leave_records = dict()
        stop_time_total = 0
        cost_list_total = [0 for _ in range(13)]
        while finish_switch:
            ...
            if log_type == "finish read file" or log_type == "start read file":
                ...
                cost_list_total = [i + j for i, j in zip(cost_list_total, log["result"][2])]
                finish_switch -= 1

        s, c = cal_leave(leave_records)
        stop_time_total += s
        cost_list_total = [i + j for i, j in zip(cost_list_total, c)]
        ...
        return (start_read_file_time, finish_read_file_time, finish_process_time), \
               (out_total, stop_time_total, cost_list_total)
    ...
    def print_result(self, time_result: tuple, cal_result: dict):
        ...
        print("\n计费统计,按月统计收入：")
        for i in range(1, 13):
            print("2019-%02d" % i + ":" + str(cal_result[2][i]) + "元")
